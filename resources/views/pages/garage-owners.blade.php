@extends('layouts.master', ['title' => 'Propriétaires garages', 'subTitle' => 'Propriétaires garages'])
...
@push('scripts')
    <script>
        ...
        function viewOwner(id) {
            fetch(`/admin/garage-owners/${id}`, {
                ...
                .then(d => {
                    ...
                    const bizHtml = (o.garages || []).map(g =>
                            `<div class="biz-row"><span><i class="fa-solid fa-wrench" style="color:#8B5CF6;margin-right:6px"></i>${g.name}</span><span style="color:var(--text-muted)">${g.city}</span></div>`
                        ).join('') ||
                        '<p style="font-size:13px;color:var(--text-muted);margin:0">Aucun garage enregistré</p>';
        ...
        function approveOwner(id) {
            postAction(`/admin/garage-owners/${id}/approve`, {}, d => {
        ...
        function suspendOwner(id) {
            postAction(`/admin/garage-owners/${id}/suspend`, {}, d => {
        ...
        function deleteOwner(id) {
            confirmAction('Supprimer ce propriétaire ? Ses garages seront aussi supprimés.', () => {
                fetch(`/admin/garage-owners/${id}`, {
                        method: 'DELETE',
                        headers: {
                            'X-CSRF-TOKEN': CSRF,
        ...
        function saveOwnerForm() {
            ...
            if (editingOwnerId) {
                fetch(`/admin/garage-owners/${editingOwnerId}`, {
                        method: 'PUT',
                        headers: {
                            'X-CSRF-TOKEN': CSRF,
                ...
                postAction('/admin/garage-owners', data, d => {
                    showToast(d.message, 'success');
                    closeModal('modalFormOwner');
                    setTimeout(() => location.reload(), 800);
                });
...
@section('content')
    <main class="page-content">
        ...
        <div class="page-hdr">
            <div>
                <h1>Propriétaires garages</h1>
                <p>Gérez les comptes propriétaires de garages partenaires.</p>
            </div>
                ...
                <table class="data-tbl">
                    ...
                    <tbody>
                        @forelse($owners as $owner)
                            <tr data-owner-id="{{ $owner->id_gara_owner }}">
                                <td>
                                ...
                                <td>{{ $owner->phone ?? '—' }}</td>
                                <td><span class="count-chip"><i class="fa-solid fa-wrench" style="font-size:9px"></i>
                                        {{ $owner->garages_count }}</span></td>
                                <td>
                                ...
                                <td>
                                    <div class="dropdown">
                                        ...
                                        <div class="dropdown-menu">
                                            <button class="dropdown-item" onclick="viewOwner({{ $owner->id_gara_owner }})">
                                                <i class="fa-solid fa-eye"></i> Voir détail
                                            </button>
                                            <button class="dropdown-item" onclick="editOwner({{ $owner->id_gara_owner }})">
                                                <i class="fa-solid fa-pen"></i> Modifier
                                            </button>
                                            @if ($owner->status === 'pending')
                                                <button class="dropdown-item"
                                                    onclick="approveOwner({{ $owner->id_gara_owner }})">
                                                    <i class="fa-solid fa-check"></i> Approuver
                                                </button>
                                            @endif
                                            <button class="dropdown-item"
                                                onclick="suspendOwner({{ $owner->id_gara_owner }})">
                                                <i
                                            ...
                                            <button class="dropdown-item text-danger"
                                                onclick="deleteOwner({{ $owner->id_gara_owner }})">
            ...
            <div class="modal-body">
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px">
                    <div style="grid-column:span 2">
                        ...
                        <input type="text" id="fName" class="form-control" placeholder="Ex: Aya Bamba">
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <input type="text" id="fCompany" class="form-control" placeholder="Ex: AutoFix SARL">
                    </div>
                    <div>
