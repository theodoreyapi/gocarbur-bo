@extends('layouts.master', ['title' => 'Affectations équipe', 'subTitle' => 'Affectations équipe'])
...
@push('scripts')
    <script>
        ...
        function editAssignment(id) {
            fetch(`/admin/team-assignments/${id}`, {
                    headers: {
        ...
        function saveAssignment() {
            ...
            if (!data.owner_id || !data.station_id) {
                showToast('Propriétaire et station requis', 'error');
                return;
            }

            if (editingId) {
                fetch(`/admin/team-assignments/${editingId}`, {
                        method: 'PUT',
                        headers: {
                            'X-CSRF-TOKEN': CSRF,
                ...
                postAction('/admin/team-assignments', data, d => {
                    showToast(d.message, 'success');
                    closeModal('modalFormAssignment');
                    setTimeout(() => location.reload(), 800);
                });
...
@section('content')
    <main class="page-content">
        ...
        <div class="page-hdr">
            <div>
                <h1>Affectations équipe</h1>
                <p>Gérez qui (propriétaire, manager, employé) est rattaché à chaque station.</p>
            </div>
        ...
        <div class="card">
            <div class="filter-bar">
                <div class="search-wrap">
                    ...
                    <input type="text" id="searchInput" class="search-inp" placeholder="Nom, email, station..."
                        value="{{ $search }}">
                </div>
                ...
                <table class="data-tbl">
                    ...
                    <tbody>
                        @forelse($assignments as $a)
                            <tr data-assignment-id="{{ $a->id_stat_owner_stat }}">
                                <td>
                                ...
                                <td>
                                    <div class="fw-600">{{ $a->station->name ?? '—' }}</div>
                                    <div style="font-size:11px;color:var(--text-muted)">{{ $a->station->city ?? '' }}</div>
                                </td>
                                <td><span
                                ...
                                <td>
                                    <div style="display:flex;gap:6px">
                                        <button class="btn btn-sm btn-secondary"
                                            onclick="editAssignment({{ $a->id_stat_owner_stat }})">
                                            <i class="fa-solid fa-pen"></i>
                                        </button>
                                        <button class="btn btn-sm btn-danger"
                                            onclick="deleteAssignment({{ $a->id_stat_owner_stat }})">
        ...
        <div class="modal-box">
            ...
            <div class="modal-body">
                <div style="margin-bottom:14px">
                    ...
                    <select id="fOwner" class="form-select" style="width:100%">
                        ...
                        @foreach ($allOwners as $o)
                            <option value="{{ $o->id_station_owner }}">{{ $o->name }} ({{ $o->email }})
                            </option>
                        @endforeach
                    </select>
                </div>
                <div style="margin-bottom:14px">
                    ...
                    <select id="fStation" class="form-select" style="width:100%">
                        <option value="">— Choisir —</option>
                        @foreach ($allStations as $st)
